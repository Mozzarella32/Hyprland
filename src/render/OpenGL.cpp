...
void CHyprOpenGLImpl::applyScreenShader(const std::string& path) {
    ...
    m_finalScreenShader.uniformLocations[SHADER_POINTER_SHAPE_PREVIOUSE]   = glGetUniformLocation(m_finalScreenShader.program, "pointer_shape_previouse");
    ...
    uniformRequireNoDamage(SHADER_POINTER_SHAPE_PREVIOUSE, "pointer_shape_previouse");
...
void CHyprOpenGLImpl::renderTextureInternal(SP<CTexture> tex, const CBox& box, const STextureRenderData& data) {
    ...
    if (usingFinalShader) {
        ...
        shader->setUniformInt(SHADER_POINTER_SHAPE_PREVIOUSE, g_pHyprRenderer->m_lastCursorData.shapePreviouse);
    }
